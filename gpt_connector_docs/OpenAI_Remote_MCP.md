...
It is very important that developers trust any remote MCP server they use with the Responses API. A malicious server can exfiltrate sensitive data from anything that enters the model's context. Carefully review the [Risks and Safety](https://platform.openai.com/docs/guides/#risks-and-safety) section below before using this tool.
...
The MCP tool works only in the [Responses API](https://platform.openai.com/docs/api-reference/responses/create), and is available across all our new models (gpt-4o, gpt-4.1, and our reasoning models). When you're using the MCP tool, you only pay for [tokens](https://platform.openai.com/docs/pricing) used when importing tool definitions or making tool calls—there are no additional fees involved.
